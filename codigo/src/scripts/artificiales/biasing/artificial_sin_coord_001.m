clear;
original = imread('C:\Users\Manuel\Documents\MATLAB\images\artificiales\biasing_con_ruido.png');
[m, n] = size(original);
image_array = zeros(m, n, 'single');
image_array(:,:) = single(original);
flat_array = image_array(:);
flat_array = normalize_array(flat_array);
[center, U, obj_fcn] = fcm_from_image_array(flat_array, 3, [2.0, 100, 1e-5,1]);
figure;
to_plot = reshape(U(1, :), n, m);
imagesc(to_plot), colorbar;
truesize;
figure;
to_plot = reshape(U(2, :), n, m);
imagesc(to_plot), colorbar;
truesize;
figure;
to_plot = reshape(U(3, :), n, m);
imagesc(to_plot), colorbar;
truesize;